...
const QualificationClientList= () =>{
  ...
  .map((p) => {
    return (
        <>
            <QualificationClient key={p.id} id={p.id} nombre={p.nombreProducto} imagenesUrl={p.producto.imagenesUrl} descripcion={p.producto.descripcion} precio={p.total} categoria={p.producto.categoria}></QualificationClient>
      </>
    ); 
  });
